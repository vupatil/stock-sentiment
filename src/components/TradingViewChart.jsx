...
export default function TradingViewChart({ symbol, interval = 'D', theme = 'dark' }) {
  ...
  useEffect(() => {
    // Validate inputs
    if (!symbol) {
      console.warn('TradingViewChart: No symbol provided');
      return;
    }
    ...
    // Clean up previous widget before creating new one
        ...
        if (widgetRef.current && widgetRef.current.onChartReady) {
          widgetRef.current.onChartReady(() => {
            try {
              ...
              console.log('All TradingView studies loaded successfully for', symbol);
            } catch (e) {
